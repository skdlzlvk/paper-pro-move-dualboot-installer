...
marker_battery=/sys/class/power_supply/elants-marker-battery/capacity
...
apply_frontlight()
{
    ...
    if printf '%s\n' "$value" >"$frontlight_brightness"; then
        ...
        log "frontlight Android=$level hardware=$value/$max"
    else
    ...
    if [ "$screen_state" = off ]; then
        ...
        if [ -n "$desired_frontlight" ] &&
           { [ "$last_screen_state" != on ] ||
             [ "$last_applied_frontlight" != "$desired_frontlight" ]; }; then
            apply_frontlight "$desired_frontlight"
            log "frontlight restored for Android screen-on"
        fi
    fi
